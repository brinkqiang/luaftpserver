...
bool FtpServerImpl::addUser(const std::string& username,
...
{
    auto ftp_session = std::make_shared<FtpSession>(io_service_,
                       ftp_users_, [this]()
    {
    ...
    asio::ip::tcp::endpoint endpoint(asio::ip::make_address(address_,
                                     make_address_ec), port_);

    if (make_address_ec)
    {
        std::cerr << "Error creating address from string \"" << address_ << "\": " <<
                  make_address_ec.message() << std::endl;
        return false;
    }
...

    {
        ...
        if (ec)
        {
            std::cerr << "Error setting reuse_address option: " << ec.message() <<
                      std::endl;
            return false;
        }
    }
...
#ifndef NDEBUG
    std::cout << "FTP Server created." << std::endl << "Listening at address " <<
              acceptor_.local_endpoint().address() << " on port " <<
...
void FtpServerImpl::acceptFtpSession(std::shared_ptr<FtpSession> ftp_session,
                                     asio::error_code const& error)
{
    ...
    std::cout << "FTP Client connected: " <<
              ftp_session->getSocket().remote_endpoint().address().to_string() << ":" <<
              ftp_session->getSocket().remote_endpoint().port() << std::endl;
#endif

    ftp_session->start();

    auto new_session = std::make_shared<FtpSession>(io_service_,
                       ftp_users_, [this]()
    {
        open_connection_count_--;
    });
...
bool FtpServerImpl::poll()
{
    int nEvent = io_service_.poll_one();
